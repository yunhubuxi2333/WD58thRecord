...
void redisCallback2(WFRedisTask* redisTask) {
    ...
    cout << "name = " << context->name << "\n";
    cout << "number = " << context->number << "\n";

    // series_of(redisTask)->set_callback([](const SeriesWork * series){
    //     SeriesContext * context = static_cast<SeriesContext
    //     *>(series->get_context()); delete context; cout << "context is
    //     deleted\n";
    // });
    series_of(redisTask)->set_callback([context](const SeriesWork* series) {
        delete context;
        cout << "context is deleted\n";
    });
    // 检查报错
    int state = redisTask->get_state();  // 获取状态
    int error = redisTask->get_error();  // 获取errno
...
int main() {
    ...
    req2->set_request("SET", {"58key", "value1"});
